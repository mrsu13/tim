...
{
    lil_dispatch_callback_proc_t proc = (lil_dispatch_callback_proc_t)lil->callback[LIL_CALLBACK_DISPATCH];
    if (proc)
        proc(lil);
}
// <-TIM
...
// TIM->
LILAPI lil_value_t lil_names(lil_t lil, const char *prefix)
{
    const size_t plen = (prefix && *prefix)
                            ? strlen(prefix)
                            : 0;

    lil_list_t names = lil_alloc_list();
    for (int i = 0; i < lil->cmds; ++i)
        if (!plen
                || strncmp(lil->cmd[i]->name, prefix, plen) == 0)
            lil_list_append(names, lil_alloc_string(lil->cmd[i]->name));

    {
        lil_env_t env = lil->env;
        while (env)
        {
            for (int i = 0; i < env->vars; ++i)
                if (!plen
                        || strncmp(env->var[i]->n, prefix, plen) == 0)
                    lil_list_append(names, lil_alloc_string(env->var[i]->n));
            env = env->parent;
        }
    }

    for (int i = 0; i < lil->rootenv->vars; ++i)
        if (!plen
                || strncmp(lil->rootenv->var[i]->n, prefix, plen) == 0)
            lil_list_append(names, lil_alloc_string(lil->rootenv->var[i]->n));

    lil_value_t r = lil_list_to_value(names, 1);
    lil_free_list(names);
    return r;
}
// <-TIM
...
{
    size_t len;
    if (lil->callback[LIL_CALLBACK_EMBEDDEDFILTER]) {
        lil_embeddedfilter_callback_proc_t proc = (lil_embeddedfilter_callback_proc_t)lil->callback[LIL_CALLBACK_EMBEDDEDFILTER];
        msg = proc(lil, msg);
        if (!msg) return;
    }
    len = strlen(msg) + 1;
