...
/**
 * Реализует семантику MQTT 3.1.1 для сопоставления топика с фильтром:
 * '+' — один уровень, '#' — остаток (включая ноль уровней). Топик
 * делится на уровни по '/' — N слэшей дают N+1 уровней (включая
 * пустые); поэтому "post/" — это два уровня: "post" и "" и должен
 * матчиться "post/+".
 */
