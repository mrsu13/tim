#include "tim_tcp_inetd.h"

#include "tim_tcp_inetd_p.h"

#include "tim_a_inetd_service.h"
#include "tim_file_tools.h"
#include "tim_trace.h"
#include "tim_translator.h"

#include "mongoose.h"

#include <cassert>


// Открытые

tim::tcp_inetd::~tcp_inetd() = default;

std::unique_ptr<tim::tcp_inetd> tim::tcp_inetd::start(mg_mgr *mg,
                                                      std::uint16_t port,
                                                      service_factory factory,
                                                      bool tls_enabled,
                                                      const std::string &if_addr)
{
    std::unique_ptr<tim::tcp_inetd> self(
        new tim::tcp_inetd(mg, port, tls_enabled, if_addr, std::move(factory)));
    if (!self->_d->_server)
        return nullptr;
    return self;
}


// Закрытые

tim::tcp_inetd::tcp_inetd(mg_mgr *mg,
                          std::uint16_t port,
                          bool tls_enabled,
                          const std::string &if_addr,
                          service_factory factory)
    : tim::inetd("tcp_inetd")
    , _d(new tim::p::tcp_inetd())
{
    assert(mg);
    assert(port && "port must be non-zero.");
    assert(factory);

    _d->_if_addr = if_addr.empty()
                        ? "0.0.0.0"
                        : if_addr;
    _d->_port = port;
    _d->_tls_enabled = tls_enabled;
    _d->_factory = std::move(factory);

    const std::string url = "tcp://" + _d->_if_addr + ":" + std::to_string(_d->_port);
    if (!(_d->_server = mg_listen(mg, url.c_str(), tim::p::tcp_inetd::handle_events, _d.get())))
        TIM_TRACE(Error,
                  TIM_TR("Failed to listen for tcp_inetd at '%s:%u'."_en,
                         "Не могу запустить tcp_inetd на '%s:%u'."_ru),
                  _d->_if_addr.c_str(), _d->_port);
}

void tim::p::tcp_inetd::handle_events(mg_connection *c, int ev, void *ev_data)
{
    tim::p::tcp_inetd *self = (tim::p::tcp_inetd *)c->fn_data;
    assert(self);

    switch (ev)
    {
        case MG_EV_OPEN:
            if (c->is_listening == 1)
                TIM_TRACE(Debug, "tcp_inetd is listening at '%s:%u'.",
                          self->_if_addr.c_str(), self->_port);
            break;

        case MG_EV_ACCEPT:
        {
            TIM_TRACE(Debug, "tcp_inetd accepted a connection at '%s:%u'.",
                      self->_if_addr.c_str(), self->_port);
            if (self->_tls_enabled)
            {
                const std::filesystem::path base_path = tim::standard_location(tim::filesystem_location::AppTlsData);
                mg_tls_opts opts =
                {
                    .ca = mg_unpacked((base_path / "ca-cert.pem").string().c_str()),
                    .cert = mg_unpacked((base_path / "cert.pem").string().c_str()),
                    .key = mg_unpacked((base_path / "key.pem").string().c_str())
                };
                mg_tls_init(c, &opts);
            }

#ifndef TIM_DEBUG
//            c->is_hexdumping = 1;
#endif

            std::unique_ptr<tim::a_inetd_service> srv = self->_factory(c);
            if (srv)
                self->_connections.emplace(c, std::move(srv));
            else
            {
                TIM_TRACE(Error,
                          TIM_TR("Failed to instantiate tcp_inetd service at '%s:%u'."_en,
                                 "Ошибка при попытке запустить сервис tcp_inetd на '%s:%u'."_ru),
                          self->_if_addr.c_str(), self->_port);
                c->is_draining = 1;
            }
            break;
        }

        case MG_EV_READ:
            if (!c->is_draining)
            {
                connection_map::const_iterator f = self->_connections.find(c);
                assert(f != self->_connections.cend());
                f->second->ready_read();
            }
            break;

        case MG_EV_CLOSE:
        {
            if (c != self->_server)
            {
                TIM_TRACE(Debug, "tcp_inetd connection at '%s:%u' closed.",
                          self->_if_addr.c_str(), self->_port);
                connection_map::iterator f = self->_connections.find(c);
                if (f != self->_connections.end())
                    self->_connections.erase(f);
            }
            else
            {
                TIM_TRACE(Debug, "tcp_inetd server at '%s:%u' closed.",
                          self->_if_addr.c_str(), self->_port);
                self->_server = nullptr;
            }
            break;
        }

        case MG_EV_ERROR:
            if (!c->is_draining)
            {
                TIM_TRACE(Error,
                          TIM_TR("Network error: %s"_en,
                                 "Сетевая ошибка: %s"_ru),
                          (char *)ev_data);
                connection_map::iterator f = self->_connections.find(c);
                assert(f != self->_connections.end());
                self->_connections.erase(f);
            }
            break;
    }
}
