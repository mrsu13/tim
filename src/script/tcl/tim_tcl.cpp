...
/**
 * Регистрирует обработчик такта между Tcl-операторами: LIL вызывает
 * его через свой DISPATCH-обработчик, чтобы во время выполнения скрипта
 * не останавливались внешние циклы событий.
 *
 * \param handler Обработчик такта.
 */
 ...
 * выполняется изолированно — внешние циклы событий не получают времени.
 */
void tim::p::tcl::dispatch(lil_t lil)
{
    // Во время eval lil_get_data возвращает tim::tcl* (устанавливается
    // в tim::tcl::eval). Вызываем зарегистрированный обработчик; если
    // он не задан, не делаем ничего — бесконечный скрипт в этом случае
    // не даст работать внешним циклам событий.
    tim::tcl *self = (tim::tcl *)lil_get_data(lil);
    if (!self)
        return;
